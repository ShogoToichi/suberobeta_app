...
const AddData = () => {
  ...
  return (
    <>
      <Title title={"スキー場情報追加フォーム"} subTitle="Tabで移動、Enterで送信、Shift＋Tabで上に移動、ひたすら繰り返せ!!これは修行だ!!"/>
      <p>スキー場名</p>
      <input type="text" onChange={doChange} value={name}></input>
      <p>都道府県</p>
      <input type="text" onChange={doChangePrefacture}value={prefecture}></input>
      <p>エリア</p>
      <input type="text" onChange={doChangeArea}value={area}></input>
      <br/>
      {/* <form onClick={doSubmit}> */}
      <button  style={{marginTop:"20px",}} onClick={doSubmit}>データべースに追加</button>
      {/* </form> */}
    </>
  )
}
